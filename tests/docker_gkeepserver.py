...
def start_docker_gkeepserver(debug_output = False):
    ...
    run_exit_code = DockerCommand("run", output=debug_output)\
        ...
        .add("-v " + this_file_dir + "/server_files/ssh:/home/keeper/.ssh")\
        .add("-v " + this_file_dir + "/server_files/config/:/home/keeper/.config/grader")\
        .add("-v " + parent_dir + "/git-keeper-core:/home/keeper/git-keeper-core")\
        .add("-v " + parent_dir + "/git-keeper-server:/home/keeper/git-keeper-server")\
    ...
    core_install_exit_code = DockerCommand('exec', output=debug_output)\
        .add('-i')\
        .add('git-keeper-server bash -c "cd /home/keeper/git-keeper-core && python3 setup.py install"')\
        .run()
    ...
    server_install_exit_code = DockerCommand('exec', output=debug_output)\
        .add('-i')\
        .add('git-keeper-server bash -c "cd /home/keeper/git-keeper-server && python3 setup.py install"')\
        .run()
