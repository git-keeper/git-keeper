...
from gkeepserver.assignments import AssignmentDirectory, send_assignment_email
...
class ResendHandler(EventHandler):
    ...
    def handle(self):
        """
        Take action after a client requests that an assignment email be resent.
        """
        ...
        try:
            ...
            if not is_published and not faculty_only:
                error = ('Assignment {} in class {} is not published.\n'
                         'Can only resend emails to the faculty account for '
            ...
            is_disabled = db.is_disabled(self._class_name,
                                         ...
                                         self._faculty_username)
            if is_disabled:
                error = ('Assignment {} in class {} is disabled, cannot '
                         'resend email'.format(self._assignment_name,
                                               self._class_name))
                raise HandlerException(error)
            ...
            for student in students:
                self._resend_email(student, assignment_dir)

            log_gkeepd_to_faculty(self._faculty_username, 'RESEND_SUCCESS',
                                  '{0} {1}'.format(self._class_name,
                                                   self._assignment_name))
            info = ('{0} resent emails for {1} for students {2}'
                    .format(self._faculty_username, self._assignment_name,
                            ' '.join(self._student_usernames)))
            gkeepd_logger.log_info(info)
        except Exception as e:
            log_gkeepd_to_faculty(self._faculty_username, 'RESEND_ERROR',
                                  str(e))
            warning = 'Resend failed: {0}'.format(e)
            gkeepd_logger.log_warning(warning)

    def _resend_email(self, student, assignment_dir: AssignmentDirectory):
        ...
        send_assignment_email(assignment_dir, student, self._faculty_username,
                              assignment_repo_path)
    ...
    def __repr__(self):
        ...
        if len(self._student_usernames) > 0:
            string = ('{0} requested to resend emails for assignment {2} in '
    ...
    def _parse_payload(self):
        ...
        if len(payload_list) < 2:
            error = ('Invalid payload for RESEND: {0}'.format(self._payload))
            raise HandlerException(error)
