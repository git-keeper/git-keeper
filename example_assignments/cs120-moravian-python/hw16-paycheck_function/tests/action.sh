...
function test_match {
    ...
    if [ $? -ne 0 ]
    then
        echo "When given these values:"
		    cat $INPUT 
		    echo "the output should be $PATTERN"
	      echo ""
...
# Run the tests
test_match "$FILEPATH" "noovertime.txt" "262.5"
test_match "$FILEPATH" "overtime.txt" "767.625"
